    ...
    return (
      (<Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          side={side}>
          <div className={`flex h-full w-full flex-col ${isMobile && 'bg-slate-50 text-slate-950'}`}>{children}</div>
        </SheetContent>
      </Sheet>)
    );
  }
...
const SidebarTrigger = React.forwardRef(({ className, onClick, ...props }, ref) => {
  ...
  return (
    (<Button
      ...
      variant="ghost"
      size="icon"
      className={cn("w-10 h-10 bg-[#f1f5f9] rounded-none", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}>
      <LuLayoutPanelLeft className="my-3" style={{ width: "2rem", height: "2rem" }} />
